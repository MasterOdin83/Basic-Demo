...
- Second API: create user, login, authorized and anonymous endpoints — Basic STS.API — register, login (JWT access token 3 min + refresh token 1 day, issued by the STS; Cloudflare Turnstile captcha on login/register; 10 attempts/min per IP), refresh, /me; no username enumeration.
...
- Unit tests for all components — 63 tests — 38 xUnit plus 25 Vitest UI tests.
- Responsive, user-friendly frontend with full CRUD — Angular 21 — login drawer, guarded routes, Bearer interceptor with silent refresh, inline editing.
